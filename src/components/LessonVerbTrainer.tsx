...
import {
  Shuffle, Eye, ChevronLeft, ChevronRight,
  CheckCircle2, XCircle, RotateCcw, BookOpen, Dumbbell,
} from "lucide-react";

export interface LessonVerb {
  infinitiv: string;
  praesens: string;
  praeteritum: string;
  perfekt: string;
...
/* ─── Flashcards ─── */
function Flashcards({ verbs }: { verbs: LessonVerb[] }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div
        ...
        className="relative min-h-[300px] sm:min-h-[340px] bg-card border border-border rounded-2xl shadow-card cursor-pointer select-none overflow-hidden transition-all duration-300 hover:shadow-elevated"
      >
          ...
          {!flipped ? (
            ...
            <div className="space-y-4 w-full max-w-xs">
              <div className="flex justify-between items-center">
                <span className="text-xs uppercase tracking-widest text-muted-foreground">Präsens (er/sie)</span>
              ...
              <div className="h-px bg-border" />
              <div className="flex justify-between items-center">
                <span className="text-xs uppercase tracking-widest text-muted-foreground">Перевод</span>
                <span className="text-base font-medium text-muted-foreground">{card.translation}</span>
              </div>
            </div>
          )}
        </div>
...
function Quiz({ verbs }: { verbs: LessonVerb[] }) {
  const [questions, setQuestions] = useState<{ verb: LessonVerb; field: QuizField }[]>([]);
  ...
  const generateQuiz = useCallback(() => {
    ...
    const pool = shuffle(verbs).map(verb => ({
      verb,
      field: fields[Math.floor(Math.random() * fields.length)],
    }));
    setQuestions(pool);
    setAnswers(Array(pool.length).fill(""));
    setChecked(Array(pool.length).fill(false));
    setScore(null);
  }, [verbs]);

  useEffect(() => { generateQuiz(); }, [generateQuiz]);

  const getCorrectAnswer = (q: { verb: LessonVerb; field: QuizField }) => q.verb[q.field];
...
interface LessonVerbTrainerProps {
  verbs: LessonVerb[];
  title?: string;
}

export function LessonVerbTrainer({ verbs, title = "Unreg. Verben — Карточки" }: LessonVerbTrainerProps) {
  ...
  return (
    <section className="animate-slide-up-fade">
      <h2 className="text-2xl font-bold tracking-display mb-2">🇩🇪 {title}</h2>
      <p className="text-sm text-muted-foreground mb-5">
        {verbs.length} глаголов • Infinitiv → Präsens → Präteritum → Perfekt
      </p>
      ...
      <div className="bg-card p-5 sm:p-8 rounded-2xl border border-border shadow-card">
        {mode === "cards" && <Flashcards verbs={verbs} />}
        {mode === "quiz" && <Quiz verbs={verbs} />}
      </div>
    </section>
  );
}
